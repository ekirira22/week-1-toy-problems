...
const NetSalary = (basicSalary, benefits) => {
    ...
    const grossSalary = basicSalary - benefitsDeductibles()

    function benefitsDeductibles(){
        return benefits + Nhif(basicSalary) + Nssf(basicSalary)
    }
    ...
    /*We are back to the mother function that now does the final deduction of the payee from the taxable income */

    return grossSalary - Payee(grossSalary)
}

// console.log(NetSalary(basicSalary, benefits))

/* Gets the element in the HTML document with id demo and appends the respective calculated salary in the innerHTML */
document.getElementById("demo").innerHTML = `Hello there, your salary will amount to ${NetSalary(basicSalary, benefits)} <br>`
